...
async function setRecordDetails(token, id){
  ...
  const receivingAccount = record.receivingAccountId === null ? undefined : await AccountsApi.getUserAccount(token, record.receivingAccountId);
  const withdrawalAccount = record.withdrawalAccountId === null ? undefined : await AccountsApi.getUserAccount(token, record.withdrawalAccountId);
